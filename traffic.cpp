            ...
            for ( ;; )
                {
                    ...
                    if ( cl.get_cmd() == 0 )
				       ...
				       for ( auto car:cars_copy )
                                    ...
                                    //iterátoros
                                    for ( std::vector<std::shared_ptr<Car>>::iterator i = cars_copy.begin(); i != cars_copy.end(); i++ )
                                        {
                                            ( * ( *i ) ).step();
				    ...
				    int getid=0;
						  ...
						  */
                                    ...
                                    //iteratoros
                                    for ( std::vector<std::shared_ptr<SmartCar>>::iterator i = m_smart_cars.begin(); i != m_smart_cars.end(); i++ )
                                        {
                                            if ( ( * ( *i ) ).get_type() == CarType::GANGSTER )
                                                {
                                                    length += std::sprintf ( data+length,
                                                                             "<OK %d %u %u %u>", cl.get_id(), ( * ( *i ) ).from(),
                                                                             ( * ( *i ) ).to_node(), ( * ( *i ) ).get_step() );
				    ...
				    for ( auto c:m_cop_cars )
                                    ...
                                    //iterátoros
                                    for ( std::vector<std::shared_ptr<CopCar>>::iterator i = m_cop_cars.begin(); i != m_cop_cars.end(); i++ )
                                        {
                                            length += std::sprintf ( data+length,
                                                                     "<OK %d %u %u %u %d>", cl.get_id(), (*(*i)).from(),
                                                                     ( * ( *i ) ).to_node(), ( * ( *i ) ).get_step(), ( * ( *i ) ).get_num_captured_gangsters() );
            ...
            if ( ( * ( *i ) ).get_type() == CarType::GANGSTER )
                {
                    ...
                    toGPS ( ( * ( *i ) ).from(), ( * ( *i ) ).to() , ( * ( *i ) ).get_step(), &lon2, &lat2 );
